 ...
 * @author Jan Stola
 */
public class SwtLayoutCodeGenerator {
   ...
   */
  public SwtLayoutCodeGenerator(LayoutModel layoutModel) {
  ...
  public void generateContainerLayout(Writer writer,
      ...
      String contVarName,
      ComponentInfo infos[]) throws IOException {
    if (contVarName == null) {
    ...
    if (interval.isGroup()) {
      ...
      if (interval.isComponent()) {
        ...
        if (min == LayoutConstants.NOT_EXPLICITLY_DEFINED) {
          // we have no minimum sizes in SWT, so use preferred size as minimun
          // intentionally commented (Case 34315): min = LayoutConstants.USE_PREFERRED_SIZE;
        ...
        int status = SwtLayoutUtils.getResizableStatus(info.clazz);
        if (!(pref == LayoutConstants.NOT_EXPLICITLY_DEFINED
            && (min == LayoutConstants.NOT_EXPLICITLY_DEFINED || min == LayoutConstants.USE_PREFERRED_SIZE
                && !info.sizingChanged
                && status == SwtLayoutUtils.STATUS_NON_RESIZABLE) && (max == LayoutConstants.NOT_EXPLICITLY_DEFINED
            || max == LayoutConstants.USE_PREFERRED_SIZE
            && !info.sizingChanged
            && status == SwtLayoutUtils.STATUS_NON_RESIZABLE || max == Short.MAX_VALUE
            && !info.sizingChanged
            && status == SwtLayoutUtils.STATUS_RESIZABLE))) {
          layout.append(", "); // NOI18N
  ...
  private void composeLinks(StringBuilder layout,
    ...
    Map<Integer, List<String>> linkSizeGroups =
        SwtLayoutUtils.createLinkSizeGroups(containerLC, dimension);
  ...
  boolean useLayoutLibrary() {
    return true /*useLayoutLibrary*/;
  }

  private String getLayoutName() {
    return useLayoutLibrary()
        ? "org.eclipse.wb.swt.layout.grouplayout.GroupLayout"
        : "javax.swing.GroupLayout"; // NOI18N
  }

  private String getLayoutStyleName() {
    return useLayoutLibrary()
        ? "org.eclipse.wb.swt.layout.grouplayout.LayoutStyle"
        : "javax.swing.LayoutStyle"; // NOI18N
  }
